                                                ...
                                                build_transient_support)
{
    ...
    valid_parameters.set("Build Electric Potential Equation",
                         false,
    ...
    _build_elec_pot_equ
        = params->get<bool>("Build Electric Potential Equation", false);
    ...
    const Teuchos::ParameterList&) const
{
    ...
    for (auto it : _equ_dof_fim_pair)
    {
        ...
        // Add source terms
        for (const auto& src : _equ_source_term.at(eq_name))
        {
            if (src.second)
            {
                add_basis_time_scalar_residual(
                    eq_name, src.first, -1.0, residual_operator_names);
            }
        }
